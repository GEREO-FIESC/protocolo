    ...
    def executar(self):
        try:
            ...
            if self.path_extrato.lower().endswith('.csv'):
                # Adicionado skiprows=5 para iniciar a leitura na 6ª linha
                df_extrato_raw = pd.read_csv(self.path_extrato, sep=None, engine='python', encoding='latin1', header=None, skiprows=5)
            else:
                # Adicionado skiprows=5 para iniciar a leitura na 6ª linha
                df_extrato_raw = pd.read_excel(self.path_extrato, header=None, skiprows=5)
